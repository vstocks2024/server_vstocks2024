...
export async function handleImageTableFieldsName(req: any, res: any, next: any) {
  try {
    if (!req) return res.send(new ErrorResponse(404, "Not Found"));
...
export async function handleDeleteImageFromBucket(req:any,resp:any,next:any){
  if(!req) new ErrorResponse(404,"No Request Found")
    try{
  const imageid=req.params.imageid;
  console.log(imageid);
  await deleteImageFileFromBucket(imageid).then(async(res:any)=>{
...
export async function handleSearchVectors(req: any, res: any, next: any) {
  ...
  try {
    ...
    await prisma.image.findMany({ where: { image_name:{ startsWith: vector_name} } }).then(async(dbresolve)=>{
        console.log(dbresolve);
